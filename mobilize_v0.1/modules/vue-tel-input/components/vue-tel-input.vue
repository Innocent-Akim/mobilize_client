<template>
  <div :class="['vue-tel-input', styleClasses, { disabled: disabled }]">
    <div :class="['vti__dropdown', { open: open, disabled: dropdownOptions.disabled }]"
      :tabindex="dropdownOptions.tabindex" @keydown="keyboardNav" @click="toggleDropdown" @keydown.esc="reset">
      <span class="vti__selection">
        <div v-if="dropdownOptions.showFlags" :class="['vti__flag', activeCountryCode.toLowerCase()]" />
      ...
      <ul ref="list" class="vti__dropdown-list" v-if="open" :class="dropdownOpenDirection">
        <li v-for="(pb, index) in sortedCountries" v-once :class="['vti__dropdown-item', getItemClass(index, pb.iso2)]"
          :key="pb.iso2 + (pb.preferred ? '-preferred' : '')" @click="choose(pb)" @mousemove="selectedIndex = index">
    ...
    <input v-model="phone" ref="input" :type="inputOptions.type" :autocomplete="inputOptions.autocomplete"
      ...
      :id="inputOptions.id" :maxlength="inputOptions.maxlength" :name="inputOptions.name"
