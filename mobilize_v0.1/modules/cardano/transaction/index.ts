...
import { serializeTreasuryRedeemer } from '../treasury-contract/redeemers'
...
export const finalizeTx = async ({
  ...
  if (scriptUtxo) {
    inputs.push(scriptUtxo) // what is the format of this scriptutxo?
  }
  ...
  if (plutusScripts) {
    ...
    const redeemerIndex = txBuilder
      ...
      .toString()
    if (bountyInfo) {
      redeemers.add(serializeTreasuryRedeemer(redeemerIndex, bountyInfo)) // JD changed this code for Bounty Treasury
    }
    if (action) {
  ...
  if (metadata) {
    ...
    generalMetadata.insert(
      Cardano.Instance.BigNum.from_str('1618'),
