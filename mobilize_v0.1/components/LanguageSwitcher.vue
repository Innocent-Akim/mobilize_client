<template>
  <div class="flex items-center">
    <Listbox v-if="currentStyle === 'dropdown-right-top'" v-model="localeSetting" as="div"
      class="relative flex items-center">
      <ListboxLabel class="sr-only">Theme</ListboxLabel>
      <ListboxButton class="btn btn-icon rounded-full bg-white btn-soft rounded-md transition duration-150 ease-in-out"
        type="button" :title="currentLang(localeSetting)">
      ...
      <ListboxOptions
        ...
        <ListboxOption v-for="lang in availableLocales" :key="lang.iso" :value="lang.iso" :class="{
    ...
    <select v-if="currentStyle === 'select-box'" v-model="localeSetting"
...
const currentLang = (code: string) => {
  return availableLocales[code].name
}
</script>
